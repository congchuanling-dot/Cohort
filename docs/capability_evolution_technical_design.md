...
> 状态：部分完成。当前已完成 P0/P4 的核心闭环：本地 capability registry、手动 gap/proposal CLI、Runner no-tool 能力缺口记录、`CapabilityGapRecorded` 观测事件、项目级 Skill scaffold、Tool/MCP adapter scaffold、doctor 诊断、依赖安装 plan/approve/install 审计、smoke test、adapter `verify/promote/enable`、`verify/promote/disable` 状态流转、available capability 轻量索引注入、重复 gap CLI 建议。Tool adapter 已有显式启用后的运行时注册，MCP adapter 仍输出明确 import 命令；更完整自动路由和离线反思汇总仍为后续规划。
...
状态：已完成项目级 Skill scaffold，并新增 `cohort capability adapter <proposal_id> --type tool|mcp` 生成 `.cohort/adapters/<capability_id>/` 下的待审查 adapter scaffold。adapter 能通过 `cohort capability verify <capability_id>` 校验 plugin manifest、README、Tool adapter `go run ./adapter.go` 或 MCP `mcp.json`，通过后才能 `promote`。`cohort capability enable <capability_id>` 会把审核后的 adapter 写入 `.cohort/capabilities/enabled_adapters.json`；Tool adapter 会在下一次 Runner 启动时注册为命令型工具，MCP adapter 会输出明确的 `cohort mcp import --scope project --merge ...`，不隐式启动外部服务。
...
> Cohort 当前具备通过 `code_run` 探测环境、运行脚本和执行依赖安装命令的基础能力，也具备 Skill、MCP、长期记忆和可观测性基础。当前版本已经能把“做不到”结构化为 capability gap，并通过 proposal、项目级 Skill scaffold 或 Tool/MCP adapter scaffold、verify、registry promote 和显式 enable 形成受控的候选能力闭环。更完整自动路由仍在后续规划中。
