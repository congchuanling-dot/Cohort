...
> Cohort 当前具备通过 `code_run` 探测环境、运行脚本和执行依赖安装命令的基础能力，也具备 Skill、MCP、长期记忆和可观测性基础。当前版本已经能把“做不到”结构化为 capability gap，并通过 proposal、项目级 Skill scaffold 或 Tool/MCP adapter scaffold、verify 和 registry promote 形成受控的候选能力闭环。adapter 自动注册和自动路由仍在后续规划中。
