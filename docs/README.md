...
12. `[部分完成]` Agent Eval：`cohort eval init/list/run/history/report`、确定性多维断言、独立评测 session、并发执行、同模型自动基线比较、临时 Fixture、最终文件状态断言、JSON 等价、文件 diff、后验命令、Git 状态断言、工具轨迹评分、启发式 Judge、重复运行稳定率、profile A/B matrix、CI gate、trace path 打通、JSON/Markdown/离线 HTML Dashboard 已落地；真实 LLM Judge 与 Dashboard 内嵌 trace timeline 待补。
