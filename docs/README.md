...
| `[部分完成]` | [capability_evolution_technical_design.md](capability_evolution_technical_design.md) | 能力边界拓展闭环方案；本地 registry、手动 gap/proposal CLI、Runner no-tool gap 记录、项目级 Skill scaffold、smoke test 和 registry promote 已落地；依赖安装审核、Tool/MCP adapter 和自动路由待实现。 |
