...
12. `[部分完成]` Agent Eval：`cohort eval init/list/run/history/report`、确定性多维断言、独立评测 session、并发执行、同 profile/model 自动基线比较、临时 Fixture、最终文件状态断言、JSON 等价、文件 diff、后验命令、Git 状态断言、工具轨迹评分、启发式 Judge、重复运行稳定率、profile A/B matrix、CI gate、trace path 打通、失败 case 内嵌 trace timeline、flaky/regression Action Items、JSON/Markdown/离线 HTML Dashboard，以及 `cohort eval stability` 历史稳定性平台已落地；真实 LLM Judge、Langfuse dataset 接入与 SQLite/daemon 化待补。
