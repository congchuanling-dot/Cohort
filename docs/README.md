...
12. `[部分完成]` Agent Eval：`cohort eval init/list/run/history/report`、确定性多维断言、独立评测 session、并发执行、自动基线比较、临时 Fixture、最终文件状态断言、工具轨迹评分、重复运行稳定率、JSON/Markdown/离线 HTML Dashboard 已落地；Judge 模型评分与多配置 A/B matrix 待补。
