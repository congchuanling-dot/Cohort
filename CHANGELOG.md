...
- Raise the default Agent `max_turns` limit from 100 to 300.

### Fixed

- Bundle macOS desktop and OCR helper scripts in the npm package so desktop Computer Use works after `npm install -g`.
- Add explicit runtime script path environment overrides for npm and advanced installs.

## [0.2.5] - 2026-07-30

### Fixed

- Use the Chrome `--args` launch path for `cohort extension open` on macOS so `chrome://extensions` opens correctly.

## [0.2.4] - 2026-07-30

### Added

- `cohort extension path` to print the local Cohort Browser Bridge Chrome extension directory.
- `cohort extension open` to open `chrome://extensions` and print the manual Load unpacked steps.
- npm packaging support for bundling the Chrome extension assets.
- GitHub installer support for installing the Chrome extension under `~/.cohort/extensions/cohort_browser_bridge`.
