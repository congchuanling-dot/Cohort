...
func TestMemoryApplyUpdateReturnsReadBackConfirmation_BitsUT(t *testing.T) {
	...
	outcome, err := tool.Run(context.Background(), agent.ToolCallContext{
		...
		Args: map[string]any{
			"candidate": map[string]any{
				"type":         "project_lesson",
				"target":       evolution.DefaultProjectMemoryPath,
				"content":      "Memory apply returns read-back confirmation fields after append.",
	...
	if data["target_path"] != filepath.Join(workspace, filepath.FromSlash(evolution.DefaultProjectMemoryPath)) {
